...
async function generateEnv() {
  try {
    ...
    console.log(`✅ Stack encontrado con ${outputs.length} outputs`);
    
    // Extraer valores necesarios
    const getOutput = (key) => {
      const output = outputs.find(o => o.OutputKey === key);
      if (!output) {
        console.warn(`⚠️  Output '${key}' no encontrado`);
        return '';
      }
    ...
    const frontendUrl = cloudFrontUrl || getOutput('FrontendUrl'); // Usar CloudFront si existe, si no S3
...
`;
    ...
    console.log(`✅ Variables de entorno generadas en: frontend/.env.production`);
    console.log(`\n📋 Resumen:`);
    ...
    // También crear .env.local para desarrollo local
...
`;
    ...
    console.log(`✅ Variables de desarrollo actualizadas en: frontend/.env.local`);
    
  } catch (error) {
    console.error('❌ Error al generar variables de entorno:', error.message);
    
    // Si es un error de credenciales, dar más información
